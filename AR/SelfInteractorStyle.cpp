...
//                seedList[i]->SetVisibility(1);
        ...
        else
        {
            ...
            vtkActor* actor = vtkActor::SafeDownCast(this->GetCurrentRenderer()->GetActors()->GetItemAsObject(1));
            vtkActor* actor1 = vtkActor::SafeDownCast(this->GetCurrentRenderer()->GetActors()->GetItemAsObject(0));
            actor->SetVisibility(0); //POINTS
            actor1->SetVisibility(0); //Regis model
            if(last_transActor != nullptr)
                last_transActor->SetVisibility(0); //Transformed model
//            for(auto i = 0; i<seedList.size();i++)
//                seedList[i]->SetVisibility(0);
        ...
        static bool showThemodel = false;
        vtkActor* actor1 = vtkActor::SafeDownCast(this->GetCurrentRenderer()->GetActors()->GetItemAsObject(0));
        if(!showThemodel)
            actor1->SetVisibility(0); //Regis model
        else if(showThemodel)
            actor1->SetVisibility(1); //Regis model
        showThemodel = !showThemodel;
    }
    else if (key == "c")
    {
        ptsType = !ptsType;
        ...
        LapaStereoRender = !LapaStereoRender;
        if(!LapaStereoRender){
            vtkMain->setTheArgcs(vtkMain->depthconfig);
        }
        else {
            vtkMain->setTheArgcs(vtkMain->lapaconfig);
        }
    }
    else if (key == "f")
    {
...
void SelfInteractorStyle::castCamera(Eigen::Matrix3d Rotate,Eigen::Vector3d translation)
{
    ...
    double wcx = -2*(VtkPose::cx-VtkPose::width/2) / (double)VtkPose::width;
    double wcy = -2*(VtkPose::cy-VtkPose::height/2) / (double)VtkPose::height;

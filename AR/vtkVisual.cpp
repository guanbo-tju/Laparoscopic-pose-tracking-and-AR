...
VtkPose::VtkPose(pcl::PointCloud<pcl::PointXYZRGBA>::Ptr pts, string config, string lapaconfig)
{
    ...
    this->depthconfig = config;
    this->lapaconfig = lapaconfig;
...
//    //init the argvs
    ...
    showLapaCamera = make_shared<thread>(bind(&LapaStereo::ImageShow,lapaC)); //show the image of the lapa
}

void VtkPose::setTheArgcs(string config){
    ...
    height= fSettings["Camera.height"];
    focusl = fSettings["FocalLength"];
    std::cout<<"Reset the parameters like fx fy cx cy"<<std::endl;
//    cond.notify_all();
}
...
void VtkPose::getpose(cv::Mat mMat)
{
    try
    {
        ...
        cv::cv2eigen(mMat,mTcw);
        {
            ...
            if(mStyle->LapaStereoRender) mTcwMat = lapaC->Tc1c2.inverse() * mTcwMat;
...
void VtkPose::combination(){
    ...
    std::unique_lock<std::mutex> lc(vtkImageDataMutex);
    if(!mStyle->LapaStereoRender)
        image_slam_ = image_slam.clone();
    else
        image_slam_ = lapaC->Gettheimage().clone();
...
//    cv::resize(image_vtk_xz,image_vtk_xz,cv::Size(width,height));
